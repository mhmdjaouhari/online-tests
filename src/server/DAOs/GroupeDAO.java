...
public class GroupeDAO {
    ...
    //find by id
    public Response search(int id_grp)
    {
        try{
            PreparedStatement pst =conn.prepareStatement("select * from groupes where id_groupe=? ;");
            pst.setInt(1,id_grp);
            ...
            if (resultSet.next()) {
                ...
                groupe.setEtudiants((ArrayList< Etudiant>)(etdDao.getAll(resultSet.getInt("id_groupe")).getData()));

                return new Response(groupe);
            } else {
                return new Response(1, "Groupe doesn't exist ");
            }
